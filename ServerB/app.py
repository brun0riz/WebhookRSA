...
APP_A_URL = "http://127.0.0.1:5000/receive_from_b"

messages_b_to_a = []
messages_received_from_a = []
...
@app_b.route('/')
def index():
    return render_template('index.html', servidor='B')

@app_b.route('/send_to_a', methods=['POST'])
def send_message_to_a():
    ...
    if message:
        ...
        try:
            ...
            payload = {'message': encrypted_message, 'sender': 'B'}
            response = requests.post(APP_A_URL, headers=headers, data=json.dumps(payload))
...
@app_b.route('/messages_sent_a', methods=['GET'])
