...
logger = logging.getLogger("ServerA")

app_a = Flask(__name__)
current_config = AppConfig(server_id='A')

keys_fully_exchanged = threading.Event()

messages_a_to_b = [] # Mensagens que A enviou para B
messages_received_from_b = [] # Mensagens que A recebeu de B
...
def inteiro_para_string(numero):
    ...
    if numero == 0 and current_config.SERVER_MODULE is not None and current_config.SERVER_MODULE > 0 : # Evitar log(0)
        return "" # Ou '\x00' se você espera caracteres nulos.
...
@app_a.route('/exchange_key', methods=['POST'])
def handle_exchange_key():
    ...
    try:
        ...
        if sender_id == current_config.OTHER_SERVER_ID:
...
def key_exchange_startup_logic():
    ...
    for attempt in range(max_retries):
        ...
        # Tenta enviar a própria chave
        sent_successfully = attempt_send_own_key()
        
        # Verifica se todas as chaves estão configuradas (inclusive a recebida do outro)
        if current_config.are_all_keys_set():
            keys_fully_exchanged.set() # Se sim, marca como completo
...
def send_message_to_other_server():
    ...
    logger.info(f"Mensagem CRIPTOGRAFADA (enviada para B): {encrypted_message}")
    timestamp = time.time()
    messages_a_to_b.append({'text': message_text, 'timestamp': timestamp})
    try:
        headers = {'Content-Type': 'application/json'}
        ...
        logger.error(f"Erro ao enviar mensagem para B: {e}", exc_info=True)
        return jsonify({'error': f'Erro ao enviar mensagem para B: {e}'}), 500

@app_a.route('/receive_message', methods=['POST'])
def receive_message_from_other_server():
    ...
    sender_id = data.get('sender_id')
    if encrypted_message is not None and sender_id == current_config.OTHER_SERVER_ID:
        ...
        timestamp = time.time()
        messages_received_from_b.append({'text': decrypted_message_text, 'timestamp': timestamp})
...
@app_a.route('/status', methods=['GET'])
def get_status():
    return jsonify({
...
if __name__ == '__main__':
    ...
    logger.info(f"Servidor A iniciando na porta {current_config.OWN_PORT}. Aguardando troca de chaves...")
    app_a.run(host='0.0.0.0', port=current_config.OWN_PORT, debug=True, use_reloader=False)